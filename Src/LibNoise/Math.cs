...
namespace LibNoise
{
  ...
  public class NMath
  {
    ...
    public static readonly double RAD_TO_DEG = 1.0 / DEG_TO_RAD;

    // These constants control certain parameters that all coherent-noise
    // functions require.
    private static int XNoiseGen = 1619;
    private static int YNoiseGen = 31337;
    private static int ZNoiseGen = 6971;
    private static int SeedNoiseGen = 1013;
    private static int ShiftNoiseGen = 8;
    ...
    public static double GradientCoherentNoise(double x, double y, double z, int seed, NoiseQuality noiseQuality)
    {
    ...
    public static double GradientNoise(double fx, double fy, double fz, int ix, int iy, int iz, long seed)
    {
    ...
    public static double ValueCoherentNoise(double x, double y, double z, int seed, NoiseQuality noiseQuality)
    {
      // Create a unit-length cube aligned along an integer boundary.  This cube
      // surrounds the input point.
      int x0 = (x > 0.0 ? (int)x : (int)x - 1);
      ...
      // interpolation.)
      double n0, n1, ix0, ix1, iy0, iy1;
      n0 = ValueNoise(x0, y0, z0, seed);
      n1 = ValueNoise(x1, y0, z0, seed);
      ix0 = LinearInterpolate(n0, n1, xs);
      n0 = ValueNoise(x0, y1, z0, seed);
      n1 = ValueNoise(x1, y1, z0, seed);
      ix1 = LinearInterpolate(n0, n1, xs);
      iy0 = LinearInterpolate(ix0, ix1, ys);

      n0 = ValueNoise(x0, y0, z1, seed);
      n1 = ValueNoise(x1, y0, z1, seed);
      ix0 = LinearInterpolate(n0, n1, xs);
      n0 = ValueNoise(x0, y1, z1, seed);
      n1 = ValueNoise(x1, y1, z1, seed);
      ix1 = LinearInterpolate(n0, n1, xs);
      iy1 = LinearInterpolate(ix0, ix1, ys);
